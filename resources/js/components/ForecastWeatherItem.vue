    ...
    <div class="container">
        <small class="container-text">
            <div class="select-style">
                <select name="timeSelector" class="select-css" id="timeSelectorForWeather" v-model="select"></select>
            </div>
        </small>
        <small class="container-text">Weather: </small>
...
<script>
    ...
    export default {
        ...
        mounted() {
            let select = document.getElementById('timeSelectorForWeather');

            let index = 0;
            this.time_data.forEach( (element) => { 
                let opt = document.createElement("option");
                opt.value = this.forecastData[index].description;
                opt.innerHTML = `${this.formatDate(element.from.date)}`;

                select.appendChild(opt);
                index++;
            });
            this.select = this.forecastData[0].description;
        },
    }
</script>
